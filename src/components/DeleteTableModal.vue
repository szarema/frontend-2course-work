<template>
  <div class="flex fixed justify-center items-center w-full h-full bg-black/80">
    <div class="flex flex-col gap-6 p-5 bg-white rounded-lg w-[500px]">
      <h1 class="text-2xl">Удалить таблицу</h1>
      ...
      <div class="flex gap-5 justify-end">
        <button @click="deleteTable" class="py-2 px-5 text-white bg-green-500 rounded-lg">
          Удалить
        </button>
...
export default {
  ...
  methods: {
    async deleteTable() {
      await api
        .delete("table/drop-table-by-name/" + this.name)
        .then((response) => {
